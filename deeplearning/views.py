...
def init(request):
    ...
    classify_info = ModelInfo.objects.create(online_url='', offline_url='', is_online=0, is_training=0,
                                             app_id=str(app_id), is_replace=2)
...
def isable_train(request):
    # return JsonResponse({'retCode': '9999', 'message': '无聊'})
    app_id = request.GET.get('appId', '-1')
    if app_id == '-1':
    ...
    if has_training_model:
        ...
        class Train_Thread(threading.Thread):
            def run(self):
                ...
                # 记录时间
                # class Timedif(keras.callbacks.Callback):
                #     def __init__(self):
                #         self.time_train_start = 0
                #         self.time_batch_end = 0
                #
                #     def on_train_begin(self, logs={}):
                #         self.time_train_start = time.time()
                #         return JsonResponse({'retCode': '99999', 'retDesc': '99999'})
                #
                #     def on_batch_end(self, batch, logs={}):
                #         self.time_batch_end = time.time()
                #         if (self.time_batch_end - self.time_train_start) > 1:
                #             ModelInfo.objects.filter(id=classify_m_info[0].id).update(is_training=0)
                #             return JsonResponse({'retCode': '1111', 'retDesc': 'time is long'})
                ...
                if length > 0:
                    if classify_score_old[1] < classify_score_train[1]:
                        ...
                        # print("发送模型")
                        ModelInfo.objects.filter(app_id=app_id, is_online=1).update(
                            online_url=online_path + 'classify' + str(app_id) + "1.h5", is_replace=1)
                    else:
...
def total_train(request):
    ...
    if has_training_model:
        ...
        if retDesc.has_key('question'):
            nb_class = retDesc['classNum'][0]['num']
        ...
        class Train_Thread(threading.Thread):
            def run(self):
                ...
                earlyStopping = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='auto')
                checkpointer = ModelCheckpoint(filepath=classify_train_cache_path, save_best_only=True, verbose=1)
                classify_model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=nb_epoch,batch_size=batch_size,shuffle=True, callbacks=[checkpointer, earlyStopping])
                ...
                cost_secends = (datetime.datetime.today() - start_time).total_seconds()
                cost_time = '%d:%d:%d' % (cost_secends / 3600, (cost_secends % 3600) / 60, cost_secends % 60)
                classify_m_history = ModelHistory.objects.filter(model_id=classify_m_info[0].id).order_by(
                    '-train_start_time')
                ModelHistory.objects.create(model_id=classify_m_info[0].id, train_data_num=len(data),
                                                accuracy=classify_score_train[1], loss=classify_score_train[0],
                                                train_start_time=start_time.strftime('%Y-%m-%d %H:%M:%S'),
